...
## 11. M9 — overall average price change — **done**

The M7 figure answers "when a price moves, by how much"; this milestone adds the
unconditioned reading requested in review: "how much does the *average* traded unit of
value lose" (D23). Same numerator — the moved fill-or-kill orders' Δsurplus — spread
over the window's whole executed fill-or-kill volume, so unmoved orders, and orders
that stop trading (whose loss is the coverage figure), count as zero price change.
Rendered as a labelled sub-row beside the conditional figure, never in its place: the
two readings differ by orders of magnitude and either alone invites misquotation.

The denominator is deliberately the **recorded** winners' executions, not our
re-arbitrated baseline (D23): it must exist for every auction, including the majority
the removed solver never bid in, and those are exactly the auctions the counterfactual
never arbitrates — arbitrating them only for a denominator would multiply the sweep's
CPU for a quantity the record already states, 5-in-7,745 divergence notwithstanding.
Computed per auction in `analyse_auction` (`recorded_executed_volume`), accumulated in
`Analysis.add` for every clean analysed auction — after the price-suspect exclusion
(a fabricated price fabricates volume too) and before the solver-present return.
Under `inherited` a reverted winner trades nothing; a missing settlement row raises
the same `MissingSettlementError` the sides do, so the denominator's coverage
requirement is exactly the analysis's own.
