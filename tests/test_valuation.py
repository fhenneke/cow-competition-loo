...
class TestPairValuesForMode:
    ...
    def test_single_pair_score_mode_is_exact(self):
        """No proxy is involved: the pair's value *is* the solution's score. This is
        what keeps every baseline exact, since only single-pair solutions set them."""
        valuation = self.make({(WETH, USDC): 100})
        values = pair_values_for_mode(valuation, "score", db_score=250)
    ...
    def test_no_contributing_orders_is_flagged(self):
        values = pair_values_for_mode(self.make({}), "score", db_score=200)
